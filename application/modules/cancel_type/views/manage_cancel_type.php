...
<section class="content-header">
    ...
    <li class="active">Manage Cancel Type</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                <div class="box-header">
                    <h3 class="box-title">List of Cancel Type</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                    <a href="#" class="btn btn-primary" id="btn-new-cancel-type"><i class="fa fa-plus"></i> Add</a>
                    <a href="#cancel_type" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                </div>
                <div class="box-body">
                    <table id="cancel-type-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Cancel Type Name </th>
                            <th>Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(isset($cancel_types) && is_array($cancel_types)){
                            foreach($cancel_types as $row){
                                ?>
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->cancel_type_name)? $row->cancel_type_name:"";?></td>
                                    ...
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>cancel_type/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#cancel-type-table').DataTable( {
            "scrollX": true,
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-cancel-type .modal-title").html("Edit Cancel Type");
            $("#cancel-type-form").attr('action', '<?php echo base_url()?>cancel_type/edit' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#cancel_type_id").val(model.cancel_type_id);
            $("#cancel_type_name").val(model.cancel_type_name);
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('cancel_type_id', model.cancel_type_id);

                $.ajax({
                    //url: '<?php //echo base_url();?>cancel_type/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
