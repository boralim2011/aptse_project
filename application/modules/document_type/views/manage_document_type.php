        ...
        <div class="col-xs-12">
            <div class="box box-info">
                <div class="box-header">
                    <h3 class="box-title">List of Document Type</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                    <a href="#" class="btn btn-primary" id="btn-new-document-type"><i class="fa fa-plus"></i> Add</a>
                    <a href="#document_type" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                </div>
                <div class="box-body">
                    <table id="document-type-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            ...
                            foreach($document_types as $row){
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>document_type/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#document-type-table').DataTable( {
            "scrollX": true,
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-document-type .modal-title").html("Edit Document Type");
            $("#document-type-form").attr('action', '<?php echo base_url()?>document_type/edit' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#document_type_id").val(model.document_type_id);
            $("#document_type_name").val(model.document_type_name);
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('document_type_id', model.document_type_id);

                $.ajax({
                    //url: '<?php //echo base_url();?>document_type/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
