...
<section class="content-header">
    ...
    <li class="active">Manage Service Type</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                <div class="box-header">
                    <h3 class="box-title">List of Service Type</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                    <a href="#" class="btn btn-primary" id="btn-new-service-type"><i class="fa fa-plus"></i> Add</a>
                    <a href="#service_type" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                </div>
                <div class="box-body">
                    <table id="service-type-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                        ...
                        <?php if(isset($service_types) && is_array($service_types)){
                            foreach($service_types as $row){
                                ...
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->service_type_name)? $row->service_type_name:"";?></td>
                                    <td><?php echo isset($row->service_type_code)? $row->service_type_code:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>service_type/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#service-type-table').DataTable( {
            "scrollX": true,
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-service-type .modal-title").html("Edit Service Type");
            $("#service-type-form").attr('action', '<?php echo base_url()?>service_type/edit' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#service_type_id").val(model.service_type_id);
            $("#service_type_name").val(model.service_type_name);
            $("#service_type_code").val(model.service_type_code);
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('service_type_id', model.service_type_id);

                $.ajax({
                    //url: '<?php //echo base_url();?>service_type/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
